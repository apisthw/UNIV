...
**Read `README.md` before making any non-trivial change.** It is the authoritative project brief (written for both humans and AI continuing the work) and is far more detailed than this file — full data schemas, per-subject content status, verified reference sources, and a running changelog. This file only covers what's needed to start working productively; README.md §1–13 has everything else, including *why* things are the way they are.
...
There is no test framework, linter, or build tool. "Testing" means syntax-checking with Node and running a mock-DOM harness that actually executes the app's data-loading and render logic — see README §10 for the full harness script. In short, after **every** edit to `index.html` or any `data/*.js` file:
...
#    and syntax-check it too. See README §10.1 for the exact extraction snippet.
...
There's no committed harness file in the repo (it's rebuilt in a scratch location per README §10.1) — read that section for the literal script to reuse instead of writing one from scratch.
...
## Critical rules (see README §2 for the full list with reasoning)
...
2. **Quiz content is original practice material, not leaked real exam questions** — but must match the *style* of real exam questions from reputable Thai tutoring institutes (verified via WebSearch/WebFetch against sources like SmartMathPro, AppliedPhysics, webythebrain), not just match the official Test Blueprint's structure/category counts. Structural correctness alone was found to be insufficient (see README §6.2).
...
- Per-subject multiple-choice option count is **not uniform** — verify against README §7/§12.1 per subject (e.g., TPAT3/TGAT1/TGAT3/A-Level English use 4 choices; A-Level Physics/Chem/Math1/Math2/TGAT2 use 5). Never assume.
